...
def __add(author, title):
    ...
    pr6_4_user.addBook(author, title)
    try:
...
def checkFind():
    ...
    while True:
        ...
        t = time.time()
        user_find = pr6_4_user.find(author, title)
        t1 = time.time()
        dt = t1 - t
...
def checkFindAuthor():
    ...
    t = time.time()
    user_find = pr6_4_user.findByAuthor(author)
    t1 = time.time()
    dt = t1 - t
    dt *= TIME_MSEC_MULTIPLIER
...
def _delete():
    ...
    while True:
        ...
        t = time.time()
        pr6_4_user.delete(author, title)
        t1 = time.time()
        dt = t1 - t
        dt *= TIME_MSEC_MULTIPLIER
...
def restoreDeleted():
    ...
    t = time.time()
    user_find = pr6_4_user.find(*pair)
    t1 = time.time()
    dt = t1 - t
    dt *= TIME_MSEC_MULTIPLIER
